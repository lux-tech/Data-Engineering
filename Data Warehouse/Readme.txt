...
2. Run the create_cluster script to set up the needed infrastructure(redshiftcluster) for this project.

    '$ python create_cluster.py' or run 'create_cluster.ipynb' in notebooks

3. Run the *create_tables* script to set up the database staging and analytical tables

    '$ python create_tables.py'  'create_tables.ipynb' - notebook
...
4. Finally, run the *etl* script to extract data from the files in S3, stage it in redshift, and finally store it in the dimensional tables.
    ...
    '$ python etl.py' or run 'etl.ipynb' notebook
...
- check_query.ipynb runs a few queries on the created star schema to validate that the project has been completed successfully.
...
- Launch a redshift cluster and create an IAM role that has read access to S3- Used create_cluster for creating cluster and assigning IAM roles to load data from S3 to redshift.
...
- To verify your query SQL loads go to query and loads under Queries and also use Editor option to connect to your database (dwh.cfg file has DB_NAME and user) and query records your create tables.
